...
`gw` 是 [go-ai-gateway](../README.md) 的个人命令行客户端——**你的个人助手，快速帮你处理简单、重复的工作**。把日常"让模型做点小事"的操作收敛到终端：
...
在 `~/.config/gw/agent.md`（`GW_STATE_DIR` 覆盖）里声明你固定的风格/规范，`gw repl` 与 `gw run` 的 agent 会话会自动把它注入 system 上下文，让模型遵循。适合放"用中文回复、代码注释用英文、提交信息格式……"这类个人约定。

```sh
cat > ~/.config/gw/agent.md <<'EOF'
用中文回复。
代码注释用英文且保持极简。
回答保持简洁,不啰嗦。
EOF
